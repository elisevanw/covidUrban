...
process_covariates <- function(countries, interventions, d, ifr.by.country,N2){
  ...
  for (Country in countries$Regions){
    ...
    interventions_region <- interventions[interventions$Country == Country, c(2,3,4,5,6,7)] # school, self-isolation, public, lockdown, social-distancing, restaurants
    ...
    df_features = data.frame('school' = school, 'selfIsolation' = selfIsolation, 'publicEvents' = publicEvents,
                             'firstIntervention' = firstIntervention, 'lockdown' = lockdown, 'socialDistancing' = socialDistancing, 'restaurantsClosed'=restaurantsClosed)
    print(df_features)
